...
- 默认的包结构
	- **主程序所在包**及其**下面的所有子包**里面的组件都会被默认扫描进来
	- 无需以前的包扫描配置
	- 想要改变扫描路径
...
public class MainApplication {
    ...
    //如果@Configuration(proxyBeanMethods = true)代理对象调用方法。SpringBoot总会检查这个组件是否在容器中有，保持单例(每次都检查，加载慢)；FALSE时不检查，加载快
        //保持组件单实例
...
引用的第三方Bean比较旧，仍然使用xml形式，同时又不想挨个写成@Bean注解形式，那么可以使用@ImportResource
